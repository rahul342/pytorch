...
{{def_kernel("A", "B", "A_inverse_scale", "B_inverse_scale")}}
    ...
    for _ in range(0, k_tiles * tiles_per_SM):
        ...
        scale_a_expanded = tl.broadcast_to(
            scale_a_expanded,
            (BLOCK_M, (BLOCK_K + 128 - 1) // 128, MIN_BLOCK_K_128)
        )  # [BLOCK_M, tl.cdiv(K, BLOCK_K), MIN_BLOCK_K_128]
        scale_a_expanded = scale_a_expanded.reshape(
            BLOCK_M,
            ((BLOCK_K + 128 - 1) // 128) * MIN_BLOCK_K_128
        )  # [BLOCK_M, MIN_BLOCK_K_128]
        ...
        scale_b_expanded = scale_b_expanded.reshape(
            ((BLOCK_N + 128 - 1) // 128) * MIN_BLOCK_N_128,
            ((BLOCK_K + 128 - 1) // 128) * MIN_BLOCK_K_128
        )  # [MIN_BLOCK_N_128, MIN_BLOCK_K_128]
